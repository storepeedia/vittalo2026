...
export async function createCamp(formData: FormData) {
  ...
  const { data, error } = await supabase.from("camps").insert({
    ...
    price_per_person: formData.get("price_per_person") ? parseFloat(formData.get("price_per_person") as string) : null,
    price_per_person_pln: formData.get("price_per_person_pln") ? parseFloat(formData.get("price_per_person_pln") as string) : null,
