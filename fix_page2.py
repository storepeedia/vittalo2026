...
# Pattern for Camp Section
camp_pattern = r'<MobileCarouselSection[\s\S]*?items=\{finalCamps\}[\s\S]*?row1Interval=\{3000\}[\s\S]*?row2Interval=\{4000\}[\s\S]*?renderItem=\{\(camp: any\) => \([\s\S]*?/>\s*\)\}\s*/>\s*<DesktopGrid[\s\S]*?items=\{finalCamps\}[\s\S]*?renderItem=\{\(camp: any\) => \([\s\S]*?/>\s*\)\}\s*/>'
...
pkg_pattern = r'<MobileCarouselSection[\s\S]*?items=\{finalPackages\}[\s\S]*?row1Interval=\{3000\}[\s\S]*?row2Interval=\{4000\}[\s\S]*?renderItem=\{\(pkg: any\) => \([\s\S]*?/>\s*\)\}\s*/>\s*<DesktopGrid[\s\S]*?items=\{finalPackages\}[\s\S]*?renderItem=\{\(pkg: any\) => \([\s\S]*?/>\s*\)\}\s*/>'
